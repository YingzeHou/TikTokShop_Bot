...
class ProductLineProcessor(BaseProcessor):
    ...
    def _get_previous_sheet_data(self, workbook, current_sheet_name):
        ...
        try:
            ...
            for row in range(2, prev_sheet.max_row + 1):
                ...
                # Cols: Sub GMV(3), Total GMV(5), Items(8), Video GMV(10), Orders(14), Impressions(17), CTR(19), CVR(21)
                metric_cols = [3, 5, 8, 10, 14, 17, 19, 21]
                metrics = {}
                ...
                if raw_id:
                    ...
                    # Derived: Video % (12)
                    metrics[12] = metrics[10] / metrics[5] if metrics[5] > 0 else 0.0
                    product_data[p_id] = metrics
                    current_group_metrics.append(metrics)
                elif "Total" in product_name:
                    # Sum basics
                    agg_metrics = {col: sum(m[col] for m in current_group_metrics) for col in [3, 5, 8, 10, 14, 17]}
                    # Average CTR and CVR for the group (approximation for WoW)
                    agg_metrics[19] = sum(m[19] for m in current_group_metrics) / len(current_group_metrics) if current_group_metrics else 0.0
                    agg_metrics[21] = sum(m[21] for m in current_group_metrics) / len(current_group_metrics) if current_group_metrics else 0.0
                    # Derived Video %
    ...
    def process(self, sheet_name="template"):
            ...
            if id_val:
                try:
                    ...
                    if p_id in stats_lookup:
                        ...
                        # [Metric Col, WoW Col]
                        mappings = [[5, 7], [8, 9], [10, 11], [14, 15], [17, 18], [19, 20], [21, 22]]
        ...
        # 7. Totals and Share Percentages
        ...
        for total_row, total_name in total_rows_meta:
            ...
            # Sum columns: 3, 5, 8, 10, 14, 17
            # And their WoW: N/A, 7, 9, 11, 15, 18
            # Plus CTR/CVR WoW: 20, 22
            agg_sum_cols = [3, 5, 8, 10, 14, 17]
            agg_wow_map = {5: 7, 8: 9, 10: 11, 14: 15, 17: 18, 19: 20, 21: 22}
            
            # Python-calculated sums/averages for WoW
            cur_agg_vals = {col: 0.0 for col in [3, 5, 8, 10, 14, 17, 19, 21]}
            p_count = 0
            ...
            if p_count > 0:
                cur_agg_vals[19] /= p_count # Average CTR
                cur_agg_vals[21] /= p_count # Average CVR
            ...
            # Inject WoW for ALL applicable columns (including CTR/CVR)
            ...
            # Aggregate Video % (12) and its WoW (13)
            new_sheet.cell(row=total_row, column=12).value = f"=IF(E{total_row}>0, J{total_row}/E{total_row}, 0)"
            this_agg_pct = cur_agg_vals[10] / cur_agg_vals[5] if cur_agg_vals[5] > 0 else 0
            if 12 in prev_agg and prev_agg[12] > 0:
                new_sheet.cell(row=total_row, column=13).value = (this_agg_pct - prev_agg[12]) / prev_agg[12]

            # Average CTR/CVR for total row (using simple average of product rows)
            new_sheet.cell(row=total_row, column=19).value = f"=AVERAGE(S{group_start}:S{total_row-1})"
            new_sheet.cell(row=total_row, column=21).value = f"=AVERAGE(U{group_start}:U{total_row-1})"

            # Share of grand total and Sub GMV % for all rows in group
        ...
        print(f"Master report fully updated with Extended WoW logic: {self.report_path}")
